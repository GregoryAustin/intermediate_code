...
public class CodegenOpt {
  ...
  private varTableOpt table;

  public CodegenOpt (TreeNode node)  {
      getProcs = true;
      table = new varTableOpt();
     this.root = node;
     basic = "";
     generateCode();
     basic = "";
  ...
  private void genCode(TreeNode currentNode) {
    switch(currentNode.tokenClass) {
      ...
      case "A"://DONE
        ...
        genCode(currentNode.getChild(2));
        String var = currentNode.getChild(0).getChild(0).getChild(0).snippet;
        String expression = currentNode.getChild(2).getChild(0).snippet;
        if (expression.equals("none")) {
          expression = currentNode.getChild(2).getChild(0).getChild(0).snippet;
        }
        table.addVar(var, expression);
        break;
      case "U"://DONE
        genCode(currentNode.getChild(0));
        break;
